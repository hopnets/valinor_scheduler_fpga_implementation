...
// Comment out the following for synthesis
//`define SIMULATION
//`define MAJOR_PRINT
//`define EXTRA_PRINT
...
(
    ...
    */
    ...
    /* output interface for dequeue(), dequeue_end(), and dequeue(f) operations */
...
`endif
    ...
    logic write_A [NUM_OF_ELEMENTS_PER_SUBLIST-1:0];
	 // $clog2: returns the logarithm with base 2
    logic [$clog2(NUM_OF_SUBLIST)-1:0] address_A [NUM_OF_ELEMENTS_PER_SUBLIST-1:0];
	 // used to write/read a complete sublist to/from the memory
    ...
    SublistElement rd_data_B [NUM_OF_ELEMENTS_PER_SUBLIST-1:0];
	 ...
	 // : rank_sublist is the name of this generator
    generate
    genvar i;
    ...
    assign s = pointer_array[s_idx_reg];
	 /*
	  * The neighbor of s to enqueue the next element in:
	  * If there isn't an element to the right of s or the element
	  * at the right side of s is full then assign the free subset as the
	  * neighbor of s to enqueue in. Otherwise, assign the subset at the right
	  * of s for the next enqueue
	 */
    ...
    assign s_neigh_enq_type = (s_idx_reg+1 < NUM_OF_SUBLIST
	 ...
	 /*
	  * We dequeue from the right sublist of s if s is full but the
	  * sublist on the right of s is neither full nor empty.
	  * Otherwise if there is a sublist at the left of s, we try
	  * dequeueing from left. Otherwise we set the neighbor to dequeue to NONE 
	  * and dequeue from s itself.
	 */
    assign s_neigh_deq = ((~dequeue_end_in) 
    ...
    assign s_neigh_deq_type = ((~dequeue_end_in)
									 & pointer_array[s_idx_reg].full
                            & s_idx_reg+1 < NUM_OF_SUBLIST
                            & ~pointer_array[s_idx_reg+1].full
                            & s_idx_reg+1 != free_list_head_reg)
	 ...
	 /*
	  * update the free sublist and elemnt moving idx
	 */
...
`endif
    ...
    always_comb begin
        for (integer i=0; i<NUM_OF_ELEMENTS_PER_SUBLIST; i=i+1) begin
            ...
            enable_AA[i] = 0;
            write_AA[i] = 0;
            address_AA[i] = '0;	// '0 fills all the bits of a vector with 0
            wr_data_AA[i] = '0;
            enable_BB[i] = 0;
            write_BB[i] = 0;
            address_BB[i] = '0;
        ...
        pred_val_deq = '0; //temp vals
		  ...
		  `ifdef EXTRA_PRINT
				$display($time, " << Checking the current state in always_comb >>");
		  `endif		  
        case(curr_state)
            RESET: begin
				 `ifdef EXTRA_PRINT
					 $display(" << The current state is: RESET >>");
					 $display("curr_address is %0d and \"NUM_OF_SUBLIST - 1\" is %0d", curr_address, NUM_OF_SUBLIST - 1);
				 `endif	
				 for (integer i=0; i<NUM_OF_ELEMENTS_PER_SUBLIST; i=i+1) begin
						  enable_A[i] = 1;
                    write_A[i] = 1;
            ...
            IDLE: begin
                ...
                if (start) begin
                    ...
                    if (enqueue_f_in) begin
							  `ifdef MAJOR_PRINT
							  	  $display("*****************************************");
								  $display($time, " << enqueue_f_in is on. Enqueueing: >>");
								  $display("f_in.id: %d", f_in.id);
								  $display("f_in.rank: %d", f_in.rank);
								  $display("f_in.send_time: %d", f_in.send_time);
							  	  $display("*****************************************");
							  `endif	
                        ...
                        for (integer i=0; i<NUM_OF_SUBLIST; i=i+1) begin
									 ...
									 `ifdef EXTRA_PRINT
										 $display("----------------------------------------------");
										 $display(" << pointer_array[i].smallest_send_time is %d >>", pointer_array[i].smallest_send_time);
										 $display(" << pointer_array[i].smallest_rank is %d >>", pointer_array[i].smallest_rank);
										 $display("----------------------------------------------");
									 `endif
                        ...
                        for (integer i=0; i<NUM_OF_SUBLIST; i=i+1) begin
									 ...
									 `ifdef EXTRA_PRINT
										 ...
										 $display(" << pointer_array[i].smallest_send_time is %d >>", pointer_array[i].smallest_send_time);
										 $display(" << pointer_array[i].smallest_rank is %d >>", pointer_array[i].smallest_rank);
										 $display(" << pointer_array[i].id is %d >>", pointer_array[i].id);
										 $display("----------------------------------------------");
                            `endif
                        ...
                        nxt_state = DEQ_FETCH_SUBLIST_FROM_MEM;
                    end else if (dequeue_end_in) begin
							  `ifdef EXTRA_PRINT
								  $display(" << dequeue_end_in is on >>");
                        ...
                        for (integer i=0; i<NUM_OF_SUBLIST; i=i+1) begin
									 ...
									 */
									 `ifdef EXTRA_PRINT
										 ...
										 $display(" << pointer_array[i].smallest_send_time is %d >>", pointer_array[i].smallest_send_time);
										 $display(" << pointer_array[i].smallest_rank is %d >>", pointer_array[i].smallest_rank);
										 $display(" << dequeue_end_largest_sublist_rank is %d >>", dequeue_end_largest_sublist_rank);
										 $display("----------------------------------------------");
									 `endif	

									 
									 /* For extracting from the tail, we don't need the 
									  * element to be eligible time wise
									  * So we don't need curr_time_in
                                    >= pointer_array[i].smallest_send_time
									  * but we need the element to exist so we need
									  * pointer_array[i].smallest_send_time != '1
									 */
									 if (pointer_array[i].smallest_send_time != '1 &&
										pointer_array[i].smallest_rank 
												>= dequeue_end_largest_sublist_rank) begin
										dequeue_end_largest_sublist_rank = pointer_array[i].smallest_rank;
										dequeue_end_largest_sublist_rank_id = pointer_array[i].id;
									 end
                        end
							  `ifdef EXTRA_PRINT
								  $display(" << dequeue_end_largest_sublist_rank is %d >>", dequeue_end_largest_sublist_rank);
								  $display(" << dequeue_end_largest_sublist_id is %d >>", dequeue_end_largest_sublist_rank_id);
							  `endif	
								
								/* 
								 * Only assign 1 to the position of the sublist with the 
								 * largest smallest_rank
								*/
								if (dequeue_end_largest_sublist_rank != '0) begin
									for (integer i=0; i<NUM_OF_SUBLIST; i=i+1) begin
										 bit_vector[i]
											= (dequeue_end_largest_sublist_rank_id == pointer_array[i].id);
									end
							  ...
							  `ifdef EXTRA_PRINT
								  $display("<< No signal for enqueue or dequeue is on >>");
							  `endif	
						  end
                end
            end
            ...
            ENQ_FETCH_SUBLIST_FROM_MEM: begin
				   `ifdef EXTRA_PRINT
					   $display("<< The current state is: ENQ_FETCH_SUBLIST_FROM_MEM >>");
				   `endif	
                if (valid_reg) begin
                    for (integer i=0; i<NUM_OF_ELEMENTS_PER_SUBLIST; i=i+1) begin
                        enable_A[i] = 1;
            ...
            POS_TO_ENQUEUE: begin
				   `ifdef EXTRA_PRINT
					   $display("<< The current state is: POS_TO_ENQUEUE >>");
				   `endif	
                if (~s_reg.full) begin
                    for (integer i=0; i<NUM_OF_ELEMENTS_PER_SUBLIST; i=i+1) begin
            ...
            ENQ_WRITE_BACK_TO_MEM: begin
				   `ifdef EXTRA_PRINT
					   $display("<< The current state is: ENQ_WRITE_BACK_TO_MEM >>");
				   `endif	
...
`endif
            ...
            DEQ_FETCH_SUBLIST_FROM_MEM: begin
				   `ifdef EXTRA_PRINT
					   $display("<< The current state is: DEQ_FETCH_SUBLIST_FROM_MEM >>");
				   `endif	
					 /*
					  * Read the sublist from the position indicated by bit_vector
					 */
                if (~valid_reg) begin
						 `ifdef EXTRA_PRINT
								$display("Valid_reg is not set (~valid_reg)");
						  `endif	
                    deq_valid_out = 1;
...
`endif
            ...
            POS_TO_DEQUEUE: begin
				   `ifdef EXTRA_PRINT
					   $display("<< The current state is: POS_TO_DEQUEUE >>");
					   $display("<< valid_reg is %d >>", valid_reg);
					   $display("<< valid_A_reg is %d >>", valid_A_reg);
				   `endif	
					 /*
					  * Sublist fetched from memory
					 */
                if (dequeue_in_reg) begin
                    for (integer i=0; i<NUM_OF_ELEMENTS_PER_SUBLIST; i=i+1) begin
								`ifdef EXTRA_PRINT
									$display("----------------------------------------------");
									$display(" << rd_data_A[i].send_time is %d >>", rd_data_A[i].send_time);
									$display(" << rd_data_A[i].rank is %d >>", rd_data_A[i].rank);
									$display("----------------------------------------------");
								`endif
                        ...
                        bit_vector_A[i] = (curr_time_in_reg >= rd_data_A[i].send_time);
						  ...
						  `ifdef EXTRA_PRINT
								$display("bit_vector_A is %b", bit_vector_A);
						  `endif
                    ...
                    for (integer i=0; i<NUM_OF_ELEMENTS_PER_SUBLIST; i=i+1) begin
								`ifdef EXTRA_PRINT
									...
									$display(" << rd_data_A[i].send_time is %d >>", rd_data_A[i].send_time);
									$display(" << rd_data_A[i].rank is %d >>", rd_data_A[i].rank);
									$display(" << rd_data_A[i].id is %d >>", rd_data_A[i].id);
									$display("----------------------------------------------");
                        `endif
						  ...
						  for (integer i=0; i<NUM_OF_ELEMENTS_PER_SUBLIST; i=i+1) begin
								 ...
								 */
								 `ifdef EXTRA_PRINT
									 ...
									 $display(" << rd_data_A[i].send_time is %d >>", rd_data_A[i].send_time);
									 $display(" << rd_data_A[i].rank is %d >>", rd_data_A[i].rank);
									 $display(" << dequeue_end_largest_element_rank is %d >>", dequeue_end_largest_element_rank);
									 $display("----------------------------------------------");
								 `endif
								 
								 /* For extracting from the tail, we don't need the 
								  * element to be eligible time wise
								  * So we don't need curr_time_in_reg
											>= rd_data_A[i].send_time
								  * but we need the element to exist so we need
								  * rd_data_A[i].send_time != '1
								 */
								 if (rd_data_A[i].send_time != '1 &&
									rd_data_A[i].rank 
											> dequeue_end_largest_element_rank) begin
									dequeue_end_largest_element_rank = rd_data_A[i].rank;
									dequeue_end_largest_element_rank_id = rd_data_A[i].id;
								 end
							end
							/* 
							 * Only assign 1 to the position of the element withing the 
							 * fetched sublist with the largest rank
							*/
							if (dequeue_end_largest_element_rank != '0) begin
								for (integer i=0; i<NUM_OF_ELEMENTS_PER_SUBLIST; i=i+1) begin
									 bit_vector_A[i]
										= (dequeue_end_largest_element_rank_id == rd_data_A[i].id);
									 `ifdef EXTRA_PRINT
										  if (dequeue_end_largest_element_rank_id == rd_data_A[i].id) begin
												$display(" << Setting bit_vector_A[%d] to 1 >>", i);
										  end
									 `endif	
								end
							end else begin
								bit_vector_A = '0;
							   `ifdef EXTRA_PRINT
									 $display("Setting all bits of bit_vector_A[i] to 0");
						  	   `endif
							end
						  ...
						  `ifdef EXTRA_PRINT
							  $display(" << dequeue_end_largest_element_rank is %d >>", dequeue_end_largest_element_rank);
							  $display(" << dequeue_end_largest_element_rank_id is %d >>", dequeue_end_largest_element_rank_id);
							  $display("bit_vector_A is %b", bit_vector_A);
						  `endif	
					 end
                ...
                if (s_neigh_type_reg != NONE) begin
						  `ifdef EXTRA_PRINT
								$display("There exists s_neigh_type_reg (s_neigh_type_reg != NONE)");
						  `endif
            ...
            DEQ_WRITE_BACK_TO_MEM: begin
				   `ifdef EXTRA_PRINT
					   $display("<< The current state is: DEQ_WRITE_BACK_TO_MEM >>");
					   $display("<< valid_reg is %d >>", valid_reg);
					   $display("<< valid_A_reg is %d >>", valid_A_reg);
				   `endif	
                if (~valid_A_reg) begin
						 `ifdef EXTRA_PRINT
								$display("valid_A_reg is not set (~valid_A_reg)");
						  `endif	
                    deq_valid_out = 1;
                    deq_element_out.id = '1;
                    deq_element_out.rank = '1;
...
`endif
                    ...
                    if (dequeue_end_in || s_neigh_type_reg == NONE) begin
                        if (valid_A_reg) begin
                            deq_valid_out = 1;
...
`endif
					...
					`ifdef MAJOR_PRINT
						$display("*****************************************");
					   $display($time, " << Dequeueing: >>");
					   $display("deq_element_out.id: %d", deq_element_out.id);
					   $display("deq_element_out.rank: %d", deq_element_out.rank);
					   $display("deq_element_out.send_time: %d", deq_element_out.send_time);
						$display("*****************************************");
				   `endif
...
`ifdef SIMULATION
            PRINT: begin
					 `ifdef EXTRA_PRINT
						$display(" << The current state is: PRINT >>");
					 `endif
                for (integer i=0; i<NUM_OF_ELEMENTS_PER_SUBLIST; i=i+1) begin
                    enable_A[i] = 1;
                    write_A[i] = 0;
            ...
            CONT_PRINTING: begin
					 `ifdef EXTRA_PRINT
						$display("<< The current state is: CONT_PRINTING >>");
					 `endif
                for (integer i=0; i<NUM_OF_ELEMENTS_PER_SUBLIST; i=i+1) begin
...
`endif
    ...
    always @(posedge clk) begin
		 `ifdef EXTRA_PRINT
		 	$display($time, " << Checking the current state in always @(posedge clk) >>");
		 `endif	
...
`endif
			  ...
			  `ifdef EXTRA_PRINT
					$display(" << rst is off >> ");
					$display(" << curr_state is %d >> ", curr_state);
			  `endif
...
`endif
                ...
                if (start) begin
                    ...
                    flow_id_in_reg <= flow_id_in;
						  dequeue_end_in_reg <= dequeue_end_in;
                    valid_reg <= valid;
                    encode_reg <= encode;
                    if (enqueue_f_in) begin
                        s_idx_reg <= (encode==0) ? encode : encode-1;
                    end else if (dequeue_in || dequeue_f_in || dequeue_end_in) begin
                        s_idx_reg <= encode;
                    end
`ifdef SIMULATION
                    curr_address <= 0;
...
`endif
				    ...
				    `ifdef EXTRA_PRINT
						  $display(" << curr_state == ENQ_FETCH_SUBLIST_FROM_MEM >> ");
				    `endif
                if (valid_reg) begin
                    s_reg <= s;
				    ...
				    `ifdef EXTRA_PRINT
						  $display(" << curr_state == POS_TO_ENQUEUE >> ");
				    `endif
                ...
                if (~s_reg.full) begin
				    ...
				    `ifdef EXTRA_PRINT
						  $display(" << curr_state == ENQ_WRITE_BACK_TO_MEM >> ");
				    `endif
                if (enqueue_case_reg == 0) begin
				    ...
				    `ifdef EXTRA_PRINT
						  $display(" << curr_state == DEQ_FETCH_SUBLIST_FROM_MEM >> ");
				    `endif
                if (~valid_reg) begin
`ifdef SIMULATION
...
`endif
				    ...
				    `ifdef EXTRA_PRINT
						  $display(" << curr_state == POS_TO_DEQUEUE >> ");
						  $display("changing valid_A_reg from %d to  %d", 
							 valid_A_reg, valid_A);
				    `endif
                valid_A_reg <= valid_A;
                encode_A_reg <= encode_A;
                ...
                if (s_reg.num == 1) begin
				    ...
				    `ifdef EXTRA_PRINT
						  $display(" << curr_state == DEQ_WRITE_BACK_TO_MEM >> ");
				    `endif
                if (~valid_A_reg) begin
`ifdef SIMULATION
                    $fwrite(f,"DEQ ELEMENT -- %0d %0d %0d\n",
...
`ifdef SIMULATION
                ...
                $fwrite(f, "\n");
				    ...
				    `ifdef EXTRA_PRINT
						  $display(" << curr_state == CONT_PRINTING >> ");
				    `endif
                cntr <= cntr + 1;
                for (integer i=0; i<NUM_OF_ELEMENTS_PER_SUBLIST; i=i+1) begin
...
/*
* If this is the proiority encoder, it gets a bit vector and reutrns the smallest index containing one
  ...
  encode_reg,valid_reg
    ...
    always_comb begin
      ...
      for(integer i=0; i<pot_width; i=i+2) begin
		...
		`ifdef EXTRA_PRINT
			$display("<< part_idx[%d] is %b >>", 0, part_idx[0]);
		`endif
    end
    ...
    generate for(lvar=1; lvar<log_width; lvar=lvar+1) begin : something
      always_comb begin
        ...
        for(integer i=0; i<pot_width; i=i+(1<<(lvar+1))) begin
		  ...
		  `ifdef EXTRA_PRINT
			  $display("<< part_idx[%d] is %b >>", lvar, part_idx[lvar]);
			  $display("<< part_idx[log_width-1][0] is %d >>", part_idx[log_width-1][0]);
			  $display("<< part_idx[log_width-1][1+:log_width] is %b >>", part_idx[log_width-1][1+:log_width]);
		  `endif
      end
    end
    endgenerate

//	 $dispaly("<< old valid_reg: %d >>", valid_reg);
//	 $dispaly("<< old encode_reg: %d >>", encode_reg);
	 
    assign valid_reg  = part_idx[log_width-1][0];
    assign encode_reg = part_idx[log_width-1][1+:log_width];
	 
//	 $dispaly("<< new valid_reg: %d >>", valid_reg);
//	 $dispaly("<< new encode_reg: %d >>", encode_reg);
endmodule
